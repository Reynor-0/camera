...
case "${color_mode}" in
    ...
    --allow-bt601-full-fallback) color_mode="bt601-full" ;;
    *)
        print_usage
        exit 2
        ;;
...
echo "Capturing one frame from ${video_device} with color mode ${color_mode} for ${duration_seconds} seconds..."
"${program}" \
...
echo "Single-frame camera display test completed."
